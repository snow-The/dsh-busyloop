...
function readStore() {
  try {
    ...
    const raw2 = JSON.parse(readFileSync(KEY_FILE, "utf8"));
    if (!Array.isArray(raw2)) return [];
    return raw2.filter((e) => e && typeof e.alias === "string" && typeof e.key === "string");
  } catch {
    return [];
  }
}
  ...
  ctx.tools?.register(
    defineTool({
      ...
      async execute(args, exec) {
        ...
        const resolved = resolveEffectiveKey(loadKey, channel.keyEnv);
  ...
  reg(defineTool({
    ...
    async execute(args) {
      try {
        ...
        const entry = addKey(String(args.alias), String(args.key), scope);
