...
export const DISCIPLINE_SYSTEM = [
  ...
  '8. 先摸清结构再深入细节;关键推断要验证;结论区分事实/推断/猜测,不把猜测当结论。大文件(>3000 行或 >1MB)禁止 read 整读:先 grep 探结构/定位,再按行号范围分块(300~800 行)读取;OCR 大文件按章节块抽取;提取前先估算全书 token 预算(行数×平均行长÷3.7),超 256K 窗口时按目标章节提取、禁止贪全。',
